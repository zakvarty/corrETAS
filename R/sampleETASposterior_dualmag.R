...
#' @param init_mag Inital Magnitude parameters at which to start the estimation (nu_0m, xi_0m, nu_1m, xi_1m).
...
#' @param mag_sds Vector of standerd deviations for Normal transition kernels in (nu_m0,xi_m0,nu_m1,xi_m1)
#' @param mu_prior Vector of parameters (alpha, beta) for the gamma prior distribution on mu
#' @author Zak Varty
#' @export
sampleETASposterior_dualmag <- function(ts, ms, m_0, t_max=NULL, init_ETAS=NULL, init_B = rep(0,length(ts)), init_mag=NULL,  sims=5000, B_samples = FALSE, B_fixed = FALSE, nuxi_t_steps=500, Calpha_steps=100, mag_steps=100, etas_sds=rep(0.1,4), mag_sds=rep(0.1,4), mu_prior=c(0.1,0.1)) {
  #--- input checks
