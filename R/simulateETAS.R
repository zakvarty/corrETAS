...
#' @param beta Parameter of the Gutenberg-Richter law used to generate all earthquake magnitudes.
#' @param gpd0 Parameter vector (sigma, xi) for generation 0 generalised pareto magnitude distribution.
#' @param gpd1 Parameter vector (sigma, xi) for aftershock generalised pareto magnitude distribution.
#' @param M0 Magnitude of completeness.
#' @param maxTime Length of the time window [0,T] to simulate the catalog over.
...
#'\dontrun{
#' beta <- 2.4; M0 <- 3
#' simulateETAS(0.2, 0.2, 1.5, 0.5, 2, beta = beta, M0 = M0, maxTime =500, displayOutput=FALSE)
#'}
...
#' @export
simulateETAS <- function(mu,K,alpha,c,p,beta=NULL,gpd0=NULL, gpd1=NULL, M0,maxTime,displayOutput=TRUE) {
  cp <-c; rm(c)


  ## Magnitude distribution checks
  #check a mainshock distribution is provided
  stopifnot(!is.null(beta) | !is.null(gpd0))
  if(!is.null(beta)){
    if (beta <= alpha || K*beta / (beta-alpha) >= 1) {
      print("warning: parameter choices will lead to supercriticality if Tmax is large enough. Infinite events may be generated")
    }
    stopifnot(beta > 0)
    gpd0 <- gpd1 <-  c(1/beta, 0)
  }
  stopifnot(gpd0[1] > 0)
  if(is.null(gpd1)){ gpd1 <- gpd0}
  ...
  rmarks0 <- function(n) {rgpd(n,scale = gpd0[1], shape= gpd0[2]) + M0}
  rmarks1 <- function(n) {rgpd(n,scale = gpd1[1], shape= gpd1[2]) + M0}
